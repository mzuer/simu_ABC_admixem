...
//declared in generateFiles.h, defined in main.cpp:
// double randomSeed;
// int numbersOfMarkers;
// int chromoSize;
// int nChromo;
// float pop1AlleleFreqAvg;
// float pop1AlleleFreqStdDev;
// float pop2AlleleFreqAvg;
// float pop2AlleleFreqStdDev;
// int nRecombMeiosis;                  // recombination file
// int factSamplePoint;
...
//================================================================================ CHANGE HERE DEFAULT FILE NAMES
string markerFile("data/markers.txt");
string recFile("data/rec.txt");
string chrFile("data/chromo.txt");
//===============================================================================================================

// FUNCTIONS DEFINED IN THIS SCRIPT
void makeAdmixemFiles();
void generateRecombinationFreqMap(int , double , double * , double * ); //called within makeAdmixemFiles
void makeCustomFiles(string) ;
void print_previous_chromo (queue<int> & , int & ,double & , ofstream &, ofstream &); // called within makeCustomFiles


/* 
 * WORKFLOW :
 * create the marker files either uniformly distributed with Admixem
 * either use gene positions previously retrieved from gff and set marker at the corresponding place
 * in parallel ouptut a file with the list of chromosomes and their length (will be needed to put the genes)
 * then create recombination map
 * so 3 output: the gene file [default: genes.txt], the recombination file [default: rec.txt] and the list of chromosomes
 *              with their length [default: chromo.txt]
 */
...
/*****************************************************************************************************************************
 * CREATE "RANDOM" FILES AS DONE BY ADMIXEM
 *****************************************************************************************************************************/
...
void makeAdmixemFiles() 
{
    ...
    int nHaploidChromosomeNum (nChromo);                  // number of haploid chromosome -> say 8 since B nigra 8n
//    double nChromosomeSize (chromoSize);                   // size of the chromosome
    ...
    string chrOutFile (chrFile);   // name of output file  // ~~ for printing the chromosome id and its length
	...
	// ~~ Initialize the size of the chromosomes -> all the same size (was not like in Admixem) but could easily be changed with an array
	while(nCurrChromosome < nHaploidChromosomeNum)
	{
		pChromosomeSizes[nCurrChromosome] = allChromoSize[nCurrChromosome];	
		/*
        ...
        */
		pCentromerePos[nCurrChromosome] = allChromoSize[nCurrChromosome];   // ~~ as implemented, not changed  (??)
		nCurrChromosome++;
    }	
	// now prepare file:
	ofstream fOutFile;
	ofstream fChrOutFile;                       // ~~ for printing the chromosome id and its length
	fChrOutFile.open(chrOutFile.c_str());       // ~~ for printing the chromosome id and its length
	...
	for (int i=0;  i < nHaploidChromosomeNum; i++) { // generate markers chromosome by chromosome
        ...
        //in chromosome file:		
    	fChrOutFile << "chr" << (i+1) << "\t" << nChrLen << endl; // ~~ print the chromosome id and its length
	...
	fOutFile.close();
	fChrOutFile.close();        // ~~ was for printing the chromosome id and its length
    // ~~ Generate the recombination probabilities
    
    cout << "...... Marker file for Admixem written in: " << sOutFile << endl;
    cout << "...... List of chromosomes and their length written in: " << chrOutFile << endl;
    
	generateRecombinationFreqMap(nHaploidChromosomeNum , nLargestChromSize, pChromosomeSizes, pCentromerePos); 
		
    delete[] pChromosomeSizes;
    delete[] pCentromerePos;
...
{ 
    ...
    cout << "...... Recombination file for Admixem written in: " << sOutFile << endl;
	
	return;
}

/*****************************************************************************************************************************
 * CREATE CUSTOM FILES BASED ON GFF DATA
 *****************************************************************************************************************************/

void makeCustomFiles(string gffFile) 
{
    // start by printing the header of the marker file (idem as Admixem)
    
    string chrOutFile (chrFile);   // name of output file  // ~~ for printing the chromosome id and its length
	ofstream ofChrOutFile;        
	ofChrOutFile.open(chrOutFile.c_str());
    string outputFile(markerFile);
	ofstream outFile;    
	outFile.open(outputFile.c_str());
	outFile << "HaploidChromosomeNum = " << nChromo << endl; 
	outFile << "RandomSeed = " << randomSeed << endl; 
	
	double nMarker = 800;                                   // !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! CHANGE !!!!!
	cout << "!! WARNING: hard-coded number of markers = " << nMarker << endl;
	double SD = nMarker * pow(0.5, 2.0) / (nMarker-1);          // THIS MIGHT BE APPROXIMATION
	
	outFile << "Pop1AvgAlleleFreq = 0.5"  << endl; 
	outFile << "Pop1AvgAlleleFreqStdev = " << SD << endl; 
	outFile << "Pop2AvgAlleleFreq = 0.5" <<  endl; 
	outFile << "Pop2AvgAlleleFreqStdev = " << SD << endl; 	
	/*
	outFile << "Pop1AvgAlleleFreq = " << pop1AlleleFreqAvg << endl; 
	outFile << "Pop1AvgAlleleFreqStdev = " << pop1AlleleFreqStdDev << endl; 
	outFile << "Pop2AvgAlleleFreq = " << pop2AlleleFreqAvg << endl; 
	outFile << "Pop2AvgAlleleFreqStdev = " << pop2AlleleFreqStdDev << endl; 	
    */
    // open the "GFF" file to parse
    ifstream inFile;
	inFile.open(gffFile.c_str());     
    queue<int> gene_pos;            // old start position of the genes in a queue
    int nLine(0);
    int chromoNbr(1);
	string chromoID;	
	string line;
	int nField;
	int chrLen;
	//int nShift(500);            // length of chromosome = nShift + position last gene CHANGE HERE  ??? <================================
	
	
	cout << "WARNING: hard-coded number of bp shift chromosome end = " << nShift << endl;
	
    //needed to run recombination map creation function
    double nLargestChromSize(0);
    double * pChromosomeSizes = new double[nChromo];           // ~~ declaration was in def.h
	double * pCentromerePos = new double[nChromo];	
    ...
    if (inFile.is_open())
    {
        ...
        {        
            ...
            {
                 //  cout << field << endl; ////////            
                if (nField == 0)   // chromosome name
                {
                    if(nLine > 0 && field != chromoID) {   // do something only if we reach the next chromosome and if it is not the first line
                        chrLen = gene_pos.back() + nShift;
                        pChromosomeSizes[chromoNbr-1] = chrLen;
                        pCentromerePos[chromoNbr-1] = chrLen/2;
                        print_previous_chromo(gene_pos, chromoNbr, nLargestChromSize, outFile, ofChrOutFile); 
                        // side effect of print_previous_chromo : chromoNbr++, if needed update nLargest
                    }
                    chromoID = field;
                } else if (nField == 1)          // this is the position, store it in the vector
                {    
                   //cout << field << endl; ////////
                    gene_pos.push(atoi(field.c_str()));
                    break;                              // do not go further, we only check the 2 first fields
                }
                nField++;
            } 
            nLine++;
        ...
        inFile.close();
        // do not forget to print the last chromo
        pChromosomeSizes[chromoNbr-1] = chrLen;
        pCentromerePos[chromoNbr-1] = chrLen/2;    
        print_previous_chromo(gene_pos, chromoNbr, nLargestChromSize, outFile, ofChrOutFile);
    }
    else cout << "Unable to open file: " << gffFile << endl ; 
    cout << "...... Marker file for Admixem written in: " << outputFile << endl;
    cout << "...... List of chromosomes and their length written in: " << chrOutFile << endl;

    // NOW CALL THE FONCTION TO CREATE RECOMBINATION MAP */
    generateRecombinationFreqMap(nChromo, nLargestChromSize, pChromosomeSizes, pCentromerePos);
...
void print_previous_chromo (queue<int> & gene_pos, int & chromoNbr, double & nLargestChromSize, ofstream & oStream, ofstream & chrStream)
{
    // ~~ Set random seed     !!! needed for NormalExt !!!!!!!!!!!!!!!
    Random::Set(randomSeed);

    // length of the chromosome, defined as nShift (500) bp after last gene position ???????????????????????????????????????????????
    int chrLen (gene_pos.back() + nShift);
    
    nLargestChromSize = (nLargestChromSize < chrLen)? chrLen : nLargestChromSize;
    
    // centromere is put at the middle of the chromosome (integer value) ?? 
    oStream << ":chr " << chromoNbr << "\tlen = " << chrLen << "\tcentromere = " << chrLen/2 << endl;
//    oStream << ":chr" << chromoNbr << "\tlen = " << chrLen << "\tcentromere = " << chrLen << endl;  // centromere at the end like Admixem??
    oStream << "\nNum\tPosition Percentage\tPosition Abs.\tFreq. Pop 1\tFreq. Pop 2" << endl ;
    
    chrStream << "chr" << chromoNbr << "\t" << chrLen << endl; // ~~ print the chromosome id and its length
    
    int i(1);
    
    bool pop1(true);
    
    // change each 10 markers between pop1 and pop2 1-0 0-1 freq
    
    while(!gene_pos.empty())
    {
        int pos(gene_pos.front());
        // number - position (relative) - position (abs) - freq pop 1 (normal random) - freq pop 2 (normal random)
        oStream << i << "\t" << pos / (float) chrLen << "\t" << pos << "\t";
        if(pop1) oStream << 1 << "\t" << 0 << endl;     // freq pop 1 = 1, freq pop 2 = 0
        else oStream << 0 << "\t" << 1 << endl;             // freq pop 1 = 0, freq pop 2 = 1
        if(i%10 == 0) pop1 = (!pop1);
        //oStream << NormalExt(pop1AlleleFreqAvg ,pop1AlleleFreqStdDev , 0.5, 1.0) << "\t" ; 
        //oStream <<  NormalExt( 1.0 - pop2AlleleFreqAvg ,pop2AlleleFreqStdDev , 0, 0.5) << endl;
...

void print_previous_chromo_ORIGINAL (queue<int> & gene_pos, int & chromoNbr, double & nLargestChromSize, ofstream & oStream, ofstream & chrStream)
{
    // ~~ Set random seed     !!! needed for NormalExt !!!!!!!!!!!!!!!
    Random::Set(randomSeed);

    // length of the chromosome, defined as 500 bp after last gene position
    int chrLen (gene_pos.back() + nShift);
