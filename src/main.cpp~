...
#include "retrieve_input.h" /*call parsing input file fction */
...
// CHANGE: is now retrieved from runABC.input
vector<double> distance_list = {0, 10, 20, 30};  // default values, will be changed  if not empty in runABC.input

// [number of populations to set at each distance]
vector<int> nbr_pop_list = {15, 15, 15, 15}; // default values, will be changed  if not empty in runABC.input

// [number of individuals to sample at each pop]
vector<int> pop_sampling_list = {20, 20, 20, 20};   // default values, will be changed  if not empty in runABC.input  
...
int nDist(1);                              // -d => number of class of distances
...
// => FOR DEBUG
string controlFile("debug_control_test.txt");    
// <= FOR DEBUG

// ./runABC -C -M <model> -n <simulations> -c <cpu> -d <nDist> -g <gff-like file> -m <marker_file> -r <rec_file> -l <chromo_file> -s <nSelGenes>]
...
gsl_rng * rd;  // random generator settings pointer, defined extern in settings.h

int main (int argc, char * const argv[]) 
{  
    // set random generator once for all
    srand(time(NULL));          
    ...
    while ((option_char = getopt (argc , argv, "ChM:n:c:d:g:m:r:l:s:")) != -1)
    {
        ...
        {  
            ...
            case 'd': nDist = atoi(optarg);break;
    ...
    if(argc == 1 || help) //help requested or no argument
    {
        ...
        cout << "./runABC -C -M <model> -n <simulations> -c <cpu> -d <nDist> -g <gff-like file> -m <marker_file> -r <rec_file> -l <chromo_file> -s <nSelGenes>]" << endl;
    ...
    if(nDist == 0 && distance_list.empty())
    {
    ...
    controlFile = model + "_"+ controlFile;
    /**/ // FOR DEBUG => 
    command = "rm -f controlFile";
...
*/
    ...
    if(makeData)
    {
        ...
        cout << "...... Running createGeneralSettings to create data" << endl;
        
        if(gffF.empty())            // no gff like file => generate the files "randomly" as Admixem did
        {
            command = "./createGeneralSettings -A";
            cout << "...... > " << command << endl;
            x = system(command.c_str());
    ...
    if(!distance_list.empty()) nDist = distance_list.size();
    
    
    for(auto i: nbr_pop_list)
    {
        cout << i << "\t";
    }
    ...
    Model current_model(model, nDist, nSelGenes);  //initialize model object
    /**/ // FOR DEBUG =>     
    ...
    for(int simu_nbr = 0; simu_nbr < nSimu; simu_nbr++)
    {
        ...
        Simu current_simu(current_model, simuID);
        #ifdef _OPENMP
            #pragma omp critical
        #endif
        {
    ...
    gsl_rng_free(rd);   // free allocation random generator
