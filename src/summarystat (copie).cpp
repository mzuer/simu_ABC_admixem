    ...
    {        
        ...
        {             
            ...
            if(nField == 1)                     // second column is the popID
            {
                ...
                if(indiv_for_freqG.empty())
                {
                    outControl << "EMPTY_TO_SAMPLE_freqG - skip: " << field << "_" << indID << endl;
                    break;
                }                
                IndivPopId currentIndiv = {field, indID};  // create a IndivPopId for the current line
                //compare the current individual with the next to sample
                //bool cmp_id( const IndivPopId & l1, const IndivPopId & l2);  // returns true if l1 < l2                                
                if(cmp_id(indiv_for_freqG.front(), currentIndiv))   //test if the one of the front is smaller
                {
                    indiv_for_freqG.pop();
                    outControl << currentIndiv.pop << "_" << currentIndiv.ind << " WAS SMALLER_freqG";
                }
                if(indiv_for_freqG.empty()) break;
                if(indiv_for_freqG.front().ind == indID &&  indiv_for_freqG.front().pop == field)
                {
                    nField++; 
                    continue; 
                } else {
            
                    outControl << field << "_" << indID << " was not to sample - SKIP_freqG" << endl;
                    break;         
                }
    ...
    {        
        ...
        while(getline(ss, field, '\t'))
        {
            ...
            if(nField == 1)                     // retrieve population ID in the 2nd column                      
            {
                ...
                if(indiv_for_freqM.empty())
                {
                    outControl << "EMPTY_TO_SAMPLE_freqM - skip: " << field << "_" << indID << endl;
                    break;
                }                
                IndivPopId currentIndiv = {field, indID};  // create a IndivPopId for the current line
                //compare the current individual with the next to sample
                //bool cmp_id( const IndivPopId & l1, const IndivPopId & l2);  // returns true if l1 < l2                                
                if(cmp_id(indiv_for_freqM.front(), currentIndiv))   //test if the one of the front is smaller
                {
                    indiv_for_freqM.pop();
                    outControl << currentIndiv.pop << "_" << currentIndiv.ind << " WAS SMALLER_freqM";
                }
                if(indiv_for_freqM.empty()) break;
                if(indiv_for_freqM.front().ind == indID &&  indiv_for_freqM.front().pop == field)
                {
                    nField++; 
                    continue;  
                } else {
                    outControl << field << "_" << indID << " was not to sample - SKIP_freqM" << endl;
                    break;                
                }
    ...
    {        
        ...
        while(getline(ss, field, '\t'))         // split the line in tab-separated fields
        {
            ...
            if(nField == 1)                     // second column is the popID
            {
                ...
                outControl << "CURRENTLY_IN " << field << "_" << indID << endl;

                // skip or update if needed
                if(indiv_for_valueG.empty())
                {
                    outControl << "EMPTY_TO_SAMPLE_valueG - skip: " << field << "_" << indID << endl;
                    break; // go to next line
                }  else if (stoi(field) > stoi(indiv_for_valueG.front().pop) &&
                        stoi(indID) > stoi(indiv_for_valueG.front().ind))
                {
                    outControl << field << "_" << indID << "BIGGER_THANvalueG" << indiv_for_valueG.front().pop <<"_" << indiv_for_valueG.front().ind << endl;
                    indiv_for_valueG.pop();
                } else if(stoi(field) == stoi(indiv_for_valueG.front().pop) && stoi(indID) > stoi(indiv_for_valueG.front().ind))
                {
                    outControl << field << "_" << indID << "BIGGER_THANvalueG" << indiv_for_valueG.front().pop <<"_" << indiv_for_valueG.front().ind << endl;
                    indiv_for_valueG.pop();
                }
                ...
                if(indiv_for_valueG.empty())
                {
                    outControl << "EMPTY_TO_SAMPLE_valueG - skip: " << field << "_" << indID << endl;
                    break; // go to next line
                } else if(stoi(indiv_for_valueG.front().ind) == stoi(indID) && stoi(indiv_for_valueG.front().pop) == stoi(field))
                {
                    outControl << "SAMPLE_valueG: " << field << "_" << indID << endl;
                    queue <string> temp_ancestryG;                  //ensure queue empty for next individual      
                    nField++; 
                    continue; // go to next field
                } else if (stoi(indiv_for_valueG.front().ind) != stoi(indID) || stoi(indiv_for_valueG.front().pop) != stoi(field))
                {
                    outControl << field << "_" << indID << " was not to sample - SKIP_valueG - hold: " << indiv_for_valueG.front().pop << "_" <<  indiv_for_valueG.front().ind << endl;
                    break; // go to next line              
                }
                outControl << "SHOULD_NEVER_BE_HERE" << endl;
    ...
    outControl << line << endl;        
        ...
        while(getline(ss, field, '\t'))         // split the line in tab-separated fields
        {
            ...
            if(nField == 1)
            {
                ...
                // skip or update if needed
                if(indiv_for_valueM.empty())
                {
                    outControl << "EMPTY_TO_SAMPLE_valueM - skip: " << field << "_" << indID << endl;
                    break; // go to next line
                }  else if (stoi(field) > stoi(indiv_for_valueM.front().pop) &&
                        stoi(indID) > stoi(indiv_for_valueM.front().ind))
                {
                    outControl << field << "_" << indID << "BIGGER_THANvalueM" << indiv_for_valueM.front().pop <<"_" << indiv_for_valueM.front().ind << endl;
                    indiv_for_valueM.pop();
                } else if(stoi(field) == stoi(indiv_for_valueM.front().pop) && stoi(indID) > stoi(indiv_for_valueM.front().ind))
                {
                    outControl << field << "_" << indID << "BIGGER_THANvalueM" << indiv_for_valueM.front().pop <<"_" << indiv_for_valueM.front().ind << endl;
                    indiv_for_valueM.pop();
                }
                ...
                if(indiv_for_valueM.empty())
                {
                    outControl << "EMPTY_TO_SAMPLE_valueM - skip: " << field << "_" << indID << endl;
                    break; // go to next line
                } else if(stoi(indiv_for_valueM.front().ind) == stoi(indID) && stoi(indiv_for_valueM.front().pop) == stoi(field))
                {
                    outControl << "SAMPLE_valueM: " << field << "_" << indID << endl;
                    queue <string> temp_ancestryG;                  //ensure queue empty for next individual      
                    nField++; 
                    continue; // go to next field
                } else if (stoi(indiv_for_valueM.front().ind) != stoi(indID) || stoi(indiv_for_valueM.front().pop) != stoi(field))
                {
                    outControl << field << "_" << indID << " was not to sample - SKIP_valueM - hold: " << indiv_for_valueM.front().pop << "_" <<  indiv_for_valueM.front().ind << endl;
                    break; // go to next line              
                }
                outControl << "SHOULD_NEVER_BE_HERE" << endl;
